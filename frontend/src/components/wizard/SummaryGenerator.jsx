...
const SummaryGenerator = ({
  ...
  // FIX: Ensure data is a string
  const [summary, setSummary] = useState(typeof data === 'string' ? data : '');
  ...
  useEffect(() => {
    // FIX: Handle both string and object data
  ...
  // FIX: Safe analyzeSummary function
  const analyzeSummary = () => {
    try {
      ...
      return { wordCount: 0, charCount: 0, keywordMatches: 0 };
    }
  };
  ...
  const handleGenerate = async () => {
    ...
    setTimeout(() => {
      ...
      const options = examples.map((example, idx) => ({
        id: idx,
        text: example
          .replace('[KEYWORD1]', safeKeywords[0] || 'technology')
          .replace('[KEYWORD2]', safeKeywords[1] || 'innovation'),
            ...
            <div className="mt-6">
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-sm font-bold text-gray-900">
                  {keywords.length > 0 ? Math.round((stats.keywordMatches / keywords.length) * 100) : 0}%
                </span>
              </div>
              <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                <motion.div
                  initial={{ width: 0 }}
                  animate={{ width: `${keywords.length > 0 ? (stats.keywordMatches / keywords.length) * 100 : 0}%` }}
